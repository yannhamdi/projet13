...
class ProductManager(models.Manager):
    def search_movie_title(self, search_entry):
        ...
        if movie_id_by_title:
            for element in movie_id_by_title:
                returning_request_user['id_code'] = element.id_code
    ...
    def search_movie_actor(self, search_actor):
        ...
        if move_id_by_actor:
            actor = move_id_by_actor[0]
            list_movie_actor  = self.filter(actor__in=actor)
            for actors in list_movie_actor:
                returning_request_user['id_code'] = actors
            return returning_request_user
        return None
